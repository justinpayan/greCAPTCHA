...
New question sets receive a UUID. Use **Load saved set** on the start screen to
paste that ID and create a new attempt without regenerating questions. The new
attempt gets its own answers, timings, scores, and optional randomized order.

The ID is not displayed anywhere in the interface, so that nothing researcher-facing
appears on screen during a session. Read it from the database instead:

```sql
SELECT id, paper_name, created_at FROM question_sets ORDER BY created_at DESC;
```
